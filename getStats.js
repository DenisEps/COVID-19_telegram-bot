...
const getCountryStats = async (country) => {
  const data = await api.getReportsByCountries(country)
  const obj = data[0][0]
  const message = `
Зарегистрировано:${obj.cases}
Смертей: ${obj.deaths}
Выздоровело: ${obj.recovered}
    `
  return {
    message, flag: obj.flag,
  }
}
...
const countries = `afghanistan, albania, algeria, andorra, angola, anguilla, antigua-and-barbuda, argentina, armenia, aruba, australia, austria, azerbaijan
...
module.exports = {
  getCountryStats, getGeneralStats, countries, capitalizeFirstLetter,
}
